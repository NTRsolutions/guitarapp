package com.demskigroup.guitaramps.device_camera;
...
public class InternalStorageContentProvider extends ContentProvider {
	...
	public boolean onCreate() {
		try {
			File mFile = new File(getContext().getFilesDir(), VariableConstants.TEMP_PHOTO_FILE_NAME);
			if(!mFile.exists()) {
				mFile.createNewFile();
	...
	public ParcelFileDescriptor openFile(Uri uri, String mode) throws FileNotFoundException {
		File f = new File(getContext().getFilesDir(), VariableConstants.TEMP_PHOTO_FILE_NAME);
		if (f.exists()) {
