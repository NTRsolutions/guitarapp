package com.demskigroup.guitaramps.pojo_class.phone_contact_pojo;

/**
 * Created by hello on 04-Jul-17.
 */

public class PhoneContactPostData
{
    /*"latitude":13.02901,
            "hashTags":"null",
            "containerHeight":"1920",
            "thumbnailImageUrl":"https://res.cloudinary.com/dxaxmyifi/image/upload/w_150,h_150/fl_progressive:steep/v1498845586/wqyyzb1ov5g7edhqpz6r.jpg",
            "postedOn":1498857587516,
            "longitude":77.589652,
            "likes":0,
            "postCaption":null,
            "containerWidth":"1080",
            "postId":1498845589167,
            "taggedUserCoordinates":null,
            "place":"10th Cross Road, RT Nagar, Bengaluru, Karnataka 560024, India",
            "usersTagged":null,
            "mainUrl":"https://res.cloudinary.com/dxaxmyifi/image/upload/fl_progressive:steep/v1498845586/wqyyzb1ov5g7edhqpz6r.jpg",
            "postLikedBy":"null",
            "hasAudio":null,
            "comments":null*/

    private String latitude="",hashTags="",containerHeight="",thumbnailImageUrl="",postedOn="",longitude="",likes="",postCaption="",containerWidth="",postId="",taggedUserCoordinates="",
            place="",usersTagged="",mainUrl="",postLikedBy="",hasAudio="",comments="";

    public String getLatitude() {
        return latitude;
    }

    public void setLatitude(String latitude) {
        this.latitude = latitude;
    }

    public String getHashTags() {
        return hashTags;
    }

    public void setHashTags(String hashTags) {
        this.hashTags = hashTags;
    }

    public String getContainerHeight() {
        return containerHeight;
    }

    public void setContainerHeight(String containerHeight) {
        this.containerHeight = containerHeight;
    }

    public String getThumbnailImageUrl() {
        return thumbnailImageUrl;
    }

    public void setThumbnailImageUrl(String thumbnailImageUrl) {
        this.thumbnailImageUrl = thumbnailImageUrl;
    }

    public String getPostedOn() {
        return postedOn;
    }

    public void setPostedOn(String postedOn) {
        this.postedOn = postedOn;
    }

    public String getLongitude() {
        return longitude;
    }

    public void setLongitude(String longitude) {
        this.longitude = longitude;
    }

    public String getLikes() {
        return likes;
    }

    public void setLikes(String likes) {
        this.likes = likes;
    }

    public String getPostCaption() {
        return postCaption;
    }

    public void setPostCaption(String postCaption) {
        this.postCaption = postCaption;
    }

    public String getContainerWidth() {
        return containerWidth;
    }

    public void setContainerWidth(String containerWidth) {
        this.containerWidth = containerWidth;
    }

    public String getPostId() {
        return postId;
    }

    public void setPostId(String postId) {
        this.postId = postId;
    }

    public String getTaggedUserCoordinates() {
        return taggedUserCoordinates;
    }

    public void setTaggedUserCoordinates(String taggedUserCoordinates) {
        this.taggedUserCoordinates = taggedUserCoordinates;
    }

    public String getPlace() {
        return place;
    }

    public void setPlace(String place) {
        this.place = place;
    }

    public String getUsersTagged() {
        return usersTagged;
    }

    public void setUsersTagged(String usersTagged) {
        this.usersTagged = usersTagged;
    }

    public String getMainUrl() {
        return mainUrl;
    }

    public void setMainUrl(String mainUrl) {
        this.mainUrl = mainUrl;
    }

    public String getPostLikedBy() {
        return postLikedBy;
    }

    public void setPostLikedBy(String postLikedBy) {
        this.postLikedBy = postLikedBy;
    }

    public String getHasAudio() {
        return hasAudio;
    }

    public void setHasAudio(String hasAudio) {
        this.hasAudio = hasAudio;
    }

    public String getComments() {
        return comments;
    }

    public void setComments(String comments) {
        this.comments = comments;
    }
}
