...
import com.demskigroup.guitaramps.event_bus.EventBusDatasHandler;
import com.demskigroup.guitaramps.main.activity.SelfProfileActivity;
import com.demskigroup.guitaramps.main.activity.UserProfileActivity;
import com.demskigroup.guitaramps.pojo_class.facebook_friends_pojo.FacebookFriendsData;
import com.demskigroup.guitaramps.pojo_class.facebook_friends_pojo.FacebookFriendsPosts;
import com.demskigroup.guitaramps.utility.ApiCall;
 ...
 *     single_row_discover_people layout and shows the friends from Facebook friends list who uses this app.
 * </p>
 * @since 04-Jul-17
 */
public class FacebookFriendsRvAdapter extends RecyclerView.Adapter<FacebookFriendsRvAdapter.MyViewHolder>
{
     ...
     * @param arrayListFbFriends The list datas
     */
    public FacebookFriendsRvAdapter(Activity mActivity, ArrayList<FacebookFriendsData> arrayListFbFriends,int followingCount) {
        this.mActivity = mActivity;
        this.arrayListFbFriends = arrayListFbFriends;
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        ...
        profilePicUrl=arrayListFbFriends.get(position).getProfilePicUrl();
        followsFlag=arrayListFbFriends.get(position).getFollowRequestStatus();
        postedByUserName=arrayListFbFriends.get(position).getMembername();
        postedByUserFullName=arrayListFbFriends.get(position).getFullname();
        ...
        final ArrayList<FacebookFriendsPosts> arrayListPostData=arrayListFbFriends.get(position).getMemberPosts();
        ...
        holder.rL_follow.setOnClickListener(new View.OnClickListener() {
            ...
            {
                String url;
                String mFollowsFlag=arrayListFbFriends.get(holder.getAdapterPosition()).getFollowRequestStatus();
                if (mFollowsFlag!=null && mFollowsFlag.equals("1"))
                {
                    ...
                    holder.tV_follow.setTextColor(ContextCompat.getColor(mActivity,R.color.purple_color));
                    holder.tV_follow.setText(mActivity.getResources().getString(R.string.follow));
                    arrayListFbFriends.get(holder.getAdapterPosition()).setFollowRequestStatus("0");
                    url=ApiUrl.UNFOLLOW+ postedByUserName;
                    if (arrayListPostData!=null && arrayListPostData.size()>0)
                        mEventBusDatasHandler.setSocialDatasFromFbFriends(arrayListPostData.get(0),false);
                ...
                else
                {
                    ...
                    holder.tV_follow.setTextColor(ContextCompat.getColor(mActivity,R.color.white));
                    holder.tV_follow.setText(mActivity.getResources().getString(R.string.Following));
                    arrayListFbFriends.get(holder.getAdapterPosition()).setFollowRequestStatus("1");
                    url=ApiUrl.FOLLOW+postedByUserName;
                    if (arrayListPostData!=null && arrayListPostData.size()>0)
                        mEventBusDatasHandler.setSocialDatasFromFbFriends(arrayListPostData.get(0),true);
                    apiCall.followUserApi(url);
                }
            }
        });
    ...
    public int getItemCount() {
        return arrayListFbFriends.size();
    }
    ...
    private void removeItem(int position) {
        arrayListFbFriends.remove(position);
        notifyItemRemoved(position);
        notifyItemRangeChanged(position, arrayListFbFriends.size());
    }
}
